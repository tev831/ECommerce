...
namespace UserActor
{
    ...
    internal class UserActor : Actor, IUserActor
    {
        ...
        public async Task AddToBasket(string productId, int quantity)
        {
            await StateManager.AddOrUpdateStateAsync(productId.ToString(), // Key (string)
                quantity, // Value
                (id, oldQuantity) => oldQuantity + quantity); // Lambda if key already exists.
        }

        public async Task ClearBasket()
        {
            IEnumerable<string> productIds = await StateManager.GetStateNamesAsync(); // Enumerates keys (aka the state).
        ...
        public async Task<BasketItem[]> GetBasket()
        {
            ...
            foreach (string productId in productIds)
            {
                ...
                results[new Guid(productId)] = quantity;
            }

            var returnVal = results.Select(kv => new BasketItem()
            {
                ProductId = kv.Key,
                Quantity = kv.Value
            }).ToArray();
